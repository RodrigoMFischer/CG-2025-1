...
glm::vec3 arcPosition(float t, float radius = 2.0f, float height = 2.0f, float centerZ = -2.0f)
{
    float angle = glm::pi<float>() * t;
    float x = radius * cos(angle);
    float y = height * sin(angle);
    return glm::vec3(x, y, centerZ);
}
...
int main()
{
		...
		Geometry geometry = setupGeometry("../assets/Modelos3D/SuzanneSubdiv1.obj");
        Geometry room = setupGeometry("../assets/Modelos3D/Cube.obj");
		...
		while (!glfwWindowShouldClose(window))
	  {
            ...
            glm::vec3 lightColor2 = (static_cast<int>(angle) % 2 == 0) ? glm::vec3(1.5f, 0.0f, 0.0f) : glm::vec3(0.0f, 0.0f, 1.5f);
			...
			glUniform3f(glGetUniformLocation(shaderID, "lightColor"), 0.5f, 0.5f, 0.5f);
			glUniform3f(glGetUniformLocation(shaderID, "cameraPos"), camera.Position.x, camera.Position.y, camera.Position.z);

            glUniform3fv(glGetUniformLocation(shaderID, "lightPos2"), 1, glm::value_ptr(lightPos2));
            glUniform3fv(glGetUniformLocation(shaderID, "lightColor2"), 1, glm::value_ptr(lightColor2));
			...
			glBindVertexArray(0);
            float t = fmod(currentTime / 4.0f, 1.0f); 
            glm::vec3 arcPos = arcPosition(t);
            glm::mat4 roomModel = glm::mat4(1.0f);
            roomModel = glm::translate(roomModel, arcPos);
            roomModel = glm::scale(roomModel, glm::vec3(0.3f));
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(roomModel));

            glActiveTexture(GL_TEXTURE0);
            if (room.textureID > 0) glBindTexture(GL_TEXTURE_2D, room.textureID);
            glUniform1i(glGetUniformLocation(shaderID, "colorBuffer"), 0);

            glBindVertexArray(room.VAO);
            glDrawArrays(GL_TRIANGLES, 0, room.vertexCount);
            glBindVertexArray(0);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
		...
		if (action == GLFW_PRESS || action == GLFW_REPEAT)
		{
				...
				if (key == GLFW_KEY_UP) camera.ProcessKeyboard("FORWARD", 0.05f);
				if (key == GLFW_KEY_DOWN) camera.ProcessKeyboard("BACKWARD", 0.05f);
				if (key == GLFW_KEY_LEFT) camera.ProcessKeyboard("LEFT", 0.05f);
				if (key == GLFW_KEY_RIGHT) camera.ProcessKeyboard("RIGHT", 0.05f);
				if (key == GLFW_KEY_PAGE_UP) camera.ProcessKeyboard("UP", 0.05f);
				if (key == GLFW_KEY_PAGE_DOWN) camera.ProcessKeyboard("DOWN", 0.05f);
				if (key == GLFW_KEY_A)	lightPos.x -= 0.2;
				if (key == GLFW_KEY_D) 	lightPos.x += 0.2;
				if (key == GLFW_KEY_S) 	lightPos.y -= 0.2;
				if (key == GLFW_KEY_W)	lightPos.y += 0.2;
				if (key == GLFW_KEY_Q)	lightPos.z -= 0.2;
				if (key == GLFW_KEY_E)	lightPos.z += 0.2;
		}
}
...
Geometry setupGeometry(const char* filepath)
{
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)0);                  // posição
    glEnableVertexAttribArray(0);

	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)(3 * sizeof(GLfloat))); // cor
    glEnableVertexAttribArray(1);

	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)(6 * sizeof(GLfloat))); // textura
    glEnableVertexAttribArray(2);

	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)(8 * sizeof(GLfloat))); // normal
    ...
    Geometry geom;
    geom.VAO = VAO;
    geom.vertexCount = vertices.size() / 11;
